...
def get_list_urls(redis):
    if redis.get('last-url-id') is not None:
        return [redis.get('url-target:'+str(key+1)).decode("utf-8")
                for key in range(int(redis.get('last-url-id')))
                if redis.get('url-target:'+str(key+1))
                ]
    else:
        print('No links')
        return 'No links'
